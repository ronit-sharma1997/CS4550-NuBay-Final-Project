...
const ItemCard = (item)  => {
	...
	<img className="img-responsive card-img-top mb-0 card-image"
    src={item.item.galleryURL}/>
	</div>
	<div className="card-body mt-0 ml-0 pl-0 mb-0 pt-0">
	<div className ="row ml-1">
			<b className="item-name"> {item.item.title} </b>
		</div>
	<div className ="row mt-0 pl-0 ml-1">
	<div className="col-12 card-subtitle mt-0 ml-0 pl-0">
		<span><b className="subTextFont">{item.item.price} </b> •
		<b className ="subTextFont"> Watch Count: {item.item.watchCount} </b></span>
	</div>
	</div>
	<div className="row">
